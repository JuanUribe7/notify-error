...
```bash
lambda-archetype$ sam local invoke HelloWorldFunction --event events/event.json
```
...
lambda-archetype$ curl http://localhost:3000/
```
...
```bash
lambda-archetype$ sam logs -n HelloWorldFunction --stack-name lambda-archetype --tail
```
...
```bash
lambda-archetype$ cd hello-world
...
```bash
sam delete --stack-name lambda-archetype
```
